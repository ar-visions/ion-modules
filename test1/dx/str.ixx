...
public:
    ...
    string(cchar_t* cstr)           : sh(shared(cstr ? new std::string(cstr)      : new std::string())) { }
    string(cchar_t* cstr, ssz len)  : sh(shared(cstr ? new std::string(cstr, len) : new std::string())) {
        if (!cstr && len) sh->reserve(len);
    }
  //string(const char      ch)      : sh(shared(new std::string(&ch, 1)))              { }
    string(ssz             sz)      : sh(shared(new std::string()))                    { sh->reserve(sz); }
...
template<> struct is_strable<string> : std::true_type  {};

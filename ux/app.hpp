...
///
/// based on the role and potential running configuration, emit and process UX
/// gotta simplify. i only want to divide it when i absolutely need to.
///
template <class V>
struct UX:Interface {
    ...
    /// -------------------------------------
    UX(int c, const char *v[], Map &defaults) {
        type     = Interface::UX;
        ///
    # if defined(ROLE_UX)
